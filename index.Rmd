...
rj_mun <- read_municipality(code_muni = 33, year = 2022) |>
          mutate(code_muni  = as.character(code_muni))
...
```{r, echo=T, out.width='90%'}
# ── 3. LEFT JOIN + TRATAR NAs ────────────────────────────────────────────────
# Partindo do mapa (left) para manter TODOS os municípios mesmo sem casos
mapa <- rj_mun |>
...
```{r, echo=T, out.width='90%'}
# 4. MAPA CONTÍNUO (GRADIENTE DE CORES)
ggplot() +
  geom_sf(data = mapa, aes(fill = incidencia), color = "white", size = 0.2) +
  ...
  labs(title = "Incidência de Casos nos Municípios do RJ - 2025",
       ...
       caption = "Fonte: Dados fornecidos") +
...
```{r, echo=T, out.width='90%'}
summary(mapa$incidencia)
```


```{r, echo=T, out.width='90%'}
# 5. MAPA CATEGÓRICO (CLASSIFICAÇÃO EM FAIXAS)
...
jenks_breaks <- classIntervals(mapa$incidencia, n = 7, style = "jenks")$brks
...
vizinhos_rainha <- poly2nb(mapa, queen = TRUE)
...
vizinhos_torre <- poly2nb(mapa, queen = FALSE)
...
```{r, echo=T, out.width='90%'}
library(sp)
coord <- coordinates(rj_sp)  # coordenadas dos centroidas dos poligonos de dourados
class(rj_sp)
```

```{r, echo=T, out.width='90%'}
viz.sf <- as(nb2lines(vizinhos_torre, coords = coord), "sf")
viz.sf <- st_set_crs(viz.sf, st_crs(rj_mun))
```
