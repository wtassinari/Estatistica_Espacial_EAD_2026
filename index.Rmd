...
```{r, echo=T,  out.width="70%"}
# Ajustando o modelo de regressão linear simples
dengue.lm <- lm(casos ~ 1, data = mapa)
summary(dengue.lm)
```
...
```{r, echo=T,  out.width="70%"}
dengue.lm$lmresid <- residuals(dengue.lm)
moran.test(dengue.lm$lmresid, pesos.viz)
```
...
```{r, echo=T,  out.width="70%"}
# library(spatialreg)
# dengue.car <- errorsarlm(casos ~ 1, data = mapa, listw = pesos.viz)
# summary(dengue.car)
```
